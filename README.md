...
# 🗄️ 데이터베이스 학습 일지 (Study Log)
> 관계형 데이터베이스 MySQL의 기본 SQL 문법을 익히고, 파이썬과 연동하여 데이터를 효율적으로 다루는 방법을 정리한 기록입니다.

### 📅 학습 목차 및 내용 SUMMARY
| 순서 | 주제 | 파일명 (.sql / .py) | 주요 학습 내용 |
| :---: | :--- | :--- | :--- |
| **01** | DBMS 및 SQL 기초 | `ex01_sql_basic.sql` | • 데이터베이스 개념 이해 및 기본 SQL 구문 (`CREATE`, `USE`, `DROP`) |
| **02** | 데이터 조회 및 조작 | `ex02_crud.sql` | • 데이터 조회 및 필터링 (`SELECT`, `WHERE`, `ORDER BY`)<br>• 데이터 삽입, 수정, 삭제 (`INSERT`, `UPDATE`, `DELETE`) |
| **03** | 집계 함수 및 조인 | `ex03_join.sql` | • 집계 및 그룹 함수 (`SUM`, `AVG`, `COUNT`, `GROUP BY`)<br>• 여러 테이블 연결 조인 (`INNER JOIN`) |
| **04** | 파이썬-MySQL 연동 | `ex04_python_mysql.py` | • `PyMySQL` 라이브러리를 활용한 DB 연결 및 쿼리 실행 |

### 💡 데이터베이스 주요 개념 정리
* **CRUD 연산**: 데이터를 생성(`INSERT`), 조회(`SELECT`), 수정(`UPDATE`), 삭제(`DELETE`)하는 데이터 처리의 4대 핵심 작업
* **테이블 조인 (`JOIN`)**: 외래키 등을 기준으로 여러 테이블을 연결하여 통합된 정보를 조회
* **파이썬 DB 연동 (`PyMySQL`)**: 파이썬 프로그램에서 직접 데이터베이스에 접속하여 SQL 쿼리를 제어하고 결과를 가공

---

# 🕷️ 웹 크롤링 학습 일지 (Study Log)
> 웹 페이지의 구조를 파악하고, BeautifulSoup과 Selenium을 활용하여 필요한 데이터를 자동으로 수집하는 기술을 정리한 기록입니다.

### 📅 학습 목차 및 내용 SUMMARY
| 순서 | 주제 | 파일명 (.ipynb / .py) | 주요 학습 내용 |
| :---: | :--- | :--- | :--- |
| **01** | 웹 크롤링 개요 | `ex01_crawling_intro.ipynb` | • 웹 크롤링 개념 이해 및 개발자 도구를 통한 HTML 구조 분석 |
| **02** | Requests와 BeautifulSoup | `ex02_bs4.ipynb` | • 정적 페이지 요청 (`requests`) 및 태그/선택자 탐색 (`select`, `find`) |
| **03** | 동적 페이지 크롤링 | `ex03_selenium.py` | • Selenium 웹 드라이버 제어 및 JavaScript 동적 로딩 데이터 수집 |
| **04** | 크롤링 실전 데이터 정제 | `ex04_crawling_practice.ipynb` | • 수집 데이터 DataFrame 변환 및 파일 저장 (`CSV`), 예외 처리 |

### 💡 웹 크롤링 주요 개념 정리
* **정적 크롤링 (`Requests + BeautifulSoup`)**: 서버에서 응답받은 정적 HTML 소스를 빠르게 파싱하여 데이터 추출
* **동적 크롤링 (`Selenium`)**: 브라우저를 직접 구동하여 사용자의 마우스 클릭, 스크롤 등 인터랙션 이후 생성되는 동적 데이터 수집
* **요소 선택자 (`CSS Selector`)**: 클래스명이나 아이디 경로를 활용해 타겟 데이터를 정확하게 지정하여 수집