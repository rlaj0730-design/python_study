...
2. **박스 모델 (Box Model)**
   - **박스 모델** : 모든 HTML 요소를 사각형 박스로 간주하며, 안쪽 여백(`padding`), 테두리(`border`), 바깥쪽 여백(`margin`)으로 구성

3. **Flexbox 레이아웃**
   - **Flexbox** : 행과 열 방향으로 요소를 유연하게 정렬하고 간격을 제어할 수 있는 현대적인 레이아웃 시스템 (`justify-content`로 수평 정렬, `align-items`로 수직 정렬)
...
1. **클라이언트 요청** : 사용자가 브라우저를 통해 특정 경로(예: `/sub`)를 요청
2. **서버 라우팅** : 서버가 `@app.route`를 통해 경로를 확인하고 알맞은 파일(예: `sub.html`)을 응답
3. **브라우저 해석 및 추가 요청** : 브라우저가 문서를 해석하다가 이미지 등의 태그를 만나면 서버에 정적 파일을 재요청 (`static/` 폴더 활용)
4. **통신 종료** : 서버가 파일을 전달하고 브라우저가 화면에 출력하면서 통신 완료