...
| 순서 | 주제 | 파일명 (.ipynb) | 주요 학습 내용 |
| --- | --- | --- | --- |
| **01** | **NumPy 기본 특징** | `ex01_numpy_기본.ipynb` | • `import numpy as np` 라이브러리 호출

• Python List vs NumPy ndarray 메모리 및 속도 비교

• 다차원(N차원) 데이터 구조 지원 |
| **02** | **배열 생성 및 속성** | `ex02_배열_생성_속성.ipynb` | • 배열 생성 (`np.array`, `np.zeros`, `np.full`, `np.arange`)

• 난수 생성 (`np.random.randint`)

• 핵심 속성 확인 (`shape`, `size`, `ndim`) |
| **03** | **연산 & 브로드캐스팅** | `ex03_배열_연산.ipynb` | • 원소별 연산 (Element-wise operations)

• 형태가 다른 배열 간 자동 확장 연산 (Broadcasting) |
| **04** | **인덱싱 & 슬라이싱** | `ex04_인덱싱_슬라이싱.ipynb` | • 다차원 인덱싱 (`arr[행, 열]`) 및 슬라이싱 (`arr[행범위, 열범위]`)

• 조건식 기반 데이터 필터링 (Boolean Indexing) |
| **05** | **데이터 분석 실습** | `ex05_numpy_실습.ipynb` | • BMI 지수 일괄 계산 실습 (단위 변환 및 공식 적용)

• 영화 평점 데이터 분석 (`np.loadtxt`, `np.unique`, 평점 필터링) |
...
| 순서 | 주제 | 파일명 (.ipynb) | 주요 학습 내용 |
| --- | --- | --- | --- |
| **01** | **Pandas 기본 구조** | `ex01_pandas_기본.ipynb` |   • `import pandas as pd` 라이브러리 호출

  • 1차원 구조 Series 및 2차원 구조 DataFrame 생성

  • DataFrame 기본 속성 (`head`, `tail`, `shape`, `info`, `describe`) |
| **02** | **데이터 선택 & 필터링** | `ex02_데이터_선택.ipynb` | • 열(Column) 및 행(Row) 선택 (`loc`, `iloc`)

• 조건식을 활용한 불리언 인덱싱 및 데이터 추출 |
| **03** | **데이터 정제 & 가공** | `ex03_데이터_정제.ipynb` |   • 결측치(NaN) 확인 및 처리 (`dropna`, `fillna`)

  • 중복 데이터 제거 및 데이터 타입(Type) 변환 |
| **04** | **그룹화 & 집계 연산** | `ex04_그룹화_연산.ipynb` | • `groupby()`를 이용한 데이터 그룹화

• 집계 함수 적용 (`mean`, `sum`, `count`, `agg`) |
| **05** | **파일 입출력 & 실습** | `ex05_pandas_실습.ipynb` |   • CSV, Excel 파일 읽기 및 쓰기 (`read_csv`, `to_csv`)

  • 실제 공공 데이터(또는 실습용 데이터셋) 전처리 및 분석 종합 실습 |
...
| 순서 | 주제 | 파일명 (.ipynb) | 주요 학습 내용 |
| --- | --- | --- | --- |
| **01** | **Matplotlib 기본 구조** | `ex01_matplotlib_기본.ipynb` | • `import matplotlib.pyplot as plt` 라이브러리 호출

• 기본 선 그래프(Line Plot) 그리기 및 `plt.show()`

• 타이틀, 축 라벨(`xlabel`, `ylabel`), 범례(`legend`) 설정 |
| **02** | **다양한 차트 시각화** | `ex02_차트_종류.ipynb` | • 산점도(`scatter`), 막대 그래프(`bar`), 히스토그램(`hist`) 작성

• 데이터 특성에 맞는 적절한 시각화 기법 선택 |
| **03** | **스타일 및 서브플롯** | `ex03_스타일_서브플롯.ipynb` | • 선 스타일, 마커, 색상(`color`, `linestyle`, `marker`) 커스텀

• `plt.subplots()`를 활용한 다중 그래프(Subplot) 배치 및 관리 |
...
| --- | --- | --- | --- |
| 01 | 기본 입출력문 | `ex01_io.html` |   • `<script>` 태그 활용
  • 입력 기능 (`prompt`, `confirm`) 및 출력 기능 (`console.log`, `alert`, `document.write`)
  • 템플릿 리터럴(Template Literal) 활용 |
| 02 | 변수 선언 키워드 | `ex02_variable.js` |   • 변수 선언 키워드 비교 (`var`, `let`, `const`)
  • 변수명 재사용 및 재할당 특성 이해 |
| 03 | 자료형 | `ex03_datatype.js` |   • 원시 자료형 (숫자형, 문자열, 논리형, `undefined`, `null`)
  • 참조 자료형 (배열 `Array`, 객체 `Object`)
  • `typeof` 연산자를 활용한 자료형 확인 |
...
| 08 | 배열 (Array) | `ex08_array.js` |   • 배열 생성, 인덱스 접근 및 주요 메서드 (`push`, `pop`, `shift`, `unshift`, `splice` 등)
  • 배열 고차 함수 (`forEach`, `map`, `filter`, `reduce`) 활용 |
| 09 | 함수 (Function) | `ex09_function.js` |   • 함수 선언문과 함수 표현식
  • 화살표 함수(Arrow Function) 문법
  • 스코프(Scope)와 클로저(Closure) 개념 |
| 10 | 객체 (Object) | `ex10_object.js` |   • 객체 생성 및 속성(Property) / 메서드(Method) 정의
  • 객체 리터럴과 `this` 키워드 이해 |
| 11 | 문서 객체 모델 (DOM) | `ex11_dom.html` |   • DOM의 개념과 노드 선택 메서드 (`getElementById`, `querySelector` 등)
  • 요소의 콘텐츠 및 스타일 제어 (`innerHTML`, `textContent`, `style`)
  • 이벤트 처리 기본 (`addEventListener`) |
| **12** | **이벤트 (Events)** | `ex12_event.html` |   • 마우스, 키보드, 폼 등 다양한 사용자 **이벤트 리스너 (`addEventListener`)** 등록
  • 이벤트 객체(`event`) 활용 및 **이벤트 전파(Bubbling & Capturing)** 제어 |
| **13** | **데이터 통신 (Async/Fetch)** | `ex13_async_fetch.html` |   • **비동기 통신(Async/Await, Promise)** 개념 이해
  • **`fetch()` API**를 활용한 외부 서버 데이터(JSON) 비동기 요청 및 응답 처리 |
...
1. **기본 입출력문과 템플릿 리터럴**
    - **입출력 함수**: `prompt`로 문자열 입력을 받고, `confirm`으로 참·거짓 선택을 받으며, `console.log`, `alert`, `document.write`로 결과 출력 가능
...
9. **객체 (Object)**
    - **객체와 `this`**: 키-값 쌍으로 데이터를 묶고, 메서드 내부에서 `this`를 통해 객체 자신의 속성에 접근
